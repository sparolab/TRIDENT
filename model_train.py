...
class Model_Train(object):
    ...
    def main_worker(self,
        ...
        if opt.gpu is not None:
            ...
            # opt.optimizer_cfg['params'] = [{'params': model.module.structure.joint_structure.parameters()},
            #                                {'params': model.module.structure.uncertainty_network.parameters()}]
    ...
    def main_train(self,
        ...
        if opt.checkpoint_path is not None:
            checkpoint = self.checkpoint_loader(opt.checkpoint_path, model, device, strict=opt.is_retrict, do_retrain=opt.retrain)
            model = checkpoint['model']
            global_step = checkpoint['global_step']
            if opt.is_triple_train == True:
                global_step = 0
        ...
        scaler = torch.cuda.amp.GradScaler(enabled=opt.do_mixed_precison) 
        ...
        while epoch < opt.num_epochs:
            ...
            for step, sample_batched in tqdm.tqdm(enumerate(dataloader.data), desc=f"Epoch: {epoch}/{opt.num_epochs}. Loop: Train", 
                ...
                if opt.distributed == True:
                    ...
                    origin = torch.autograd.Variable(sample_batched['origin'].cuda(device, non_blocking=True))
                    ...
                    origin = torch.autograd.Variable(sample_batched['origin'].to(torch.device("cpu")))
                
                if opt.is_triple_train is False: 
                    depth_est, depth_scale_list, second_predicted, _, _ = model(image)
                    depth_16x16_scaled, depth_8x8_scaled, depth_4x4_scaled, depth_2x2_scaled = depth_scale_list

                elif opt.is_triple_train is True: 
                    depth_est, _, uncertainty_est, second_predicted, _, _ = model(image)

                
                valid_mask = torch.logical_and(depth_gt > opt.min_depth_eval, depth_gt < opt.max_depth_eval)
                
                err_gt = torch.abs(depth_est - depth_gt)
                err_gt[err_gt < opt.min_depth_eval] = opt.min_depth_eval
                                
                if opt.is_triple_train is False:

                    enhanced_est_list = [[second_predicted, enhanced_gt, valid_mask],
                                         [second_predicted, enhanced_gt, valid_mask]]
                                        
                    depth_est_list = [[depth_est, depth_gt, valid_mask]
                                    ]

                    loss = criterion.forward([enhanced_est_list, depth_est_list])

                elif opt.is_triple_train is True:
                                            
                    enhanced_est_list = [[second_predicted.detach(), enhanced_gt.detach(), valid_mask],
                                         [second_predicted.detach(), enhanced_gt.detach(), valid_mask]]
                    depth_est_list = [[depth_est.detach(), depth_gt.detach(), valid_mask]
                                    ]
                    loss = criterion.forward([enhanced_est_list, depth_est_list])
                ...
                if opt.is_triple_train is True:
                    uncertainty_gt_tmp = torch.abs(depth_est.detach() - depth_gt.detach()) / opt.max_depth
                    final_loss = 512.0 * torch.mean(torch.abs(uncertainty_est[valid_mask]-uncertainty_gt_tmp[valid_mask])**2)
                ...
                if global_step and global_step % opt.log_freq == 0 and not model_loaded_sucess:      
                    ...
                    if opt.do_use_logger == 'Wandb' and (not opt.multiprocessing_distributed or (opt.multiprocessing_distributed and opt.rank % ngpus_per_node == 0)):
                        ...
                        train_logger.logging_images('Depth Output Images', depth_image_list, depth_image_tag_list, global_step)
                        
                        enhanced_image_tag_list = ['enhanced_gt', 'enhanced_est', 'enhanced_original']
                        enhanced_image_list = [enhanced_gt[0, :, :, :].data, second_predicted[0, :, :, :].data, uw_inv_normalize(image[0, :, :, :]).data]
                        train_logger.logging_images('Enhanced Output Images', enhanced_image_list, enhanced_image_tag_list, global_step)
                    ...
                    if opt.do_use_logger == 'Tensorboard' and (not opt.multiprocessing_distributed or (opt.multiprocessing_distributed and opt.rank % ngpus_per_node == 0)):
                        ...
                        if opt.is_triple_train == False:
                            image_tag_list = ['depth_8x8', 'depth_4x4', 'depth_2x2']
                            image_list = [
                                        normalize_result(1/depth_8x8_scaled[0, :, :, :].data), 
                                        normalize_result(1/depth_4x4_scaled[0, :, :, :].data), 
                                        normalize_result(1/depth_2x2_scaled[0, :, :, :].data)]
                            train_logger.image_summary(image_tag_list, image_list, global_step)

                        if opt.is_triple_train is True:
                            uncertainty_max = torch.max(uncertainty_est[0])
                            uncertainty_min = torch.min(uncertainty_est[0])
                            uncert_value = final_loss
                            train_logger.scalar_summary(['Uncertainty_max', 'Uncertainty_min','Uncert_value'], [uncertainty_max,uncertainty_min, uncert_value], global_step)
                                                
                            err_gt = torch.where(err_gt < 1e-3, err_gt * 0 + 1e3, err_gt)
                            err_gt[torch.isnan(err_gt)] = opt.min_depth_eval
                            err_gt[torch.isinf(err_gt)] = opt.max_depth_eval

                            uncertainty_image_tag_list = ['uncertainty_est', 'uncertain_err_gt', 'uncertainty_est_real']
                                                        
                            uncertainty_est_real = uncertainty_est * depth_est
                            uncertainty_est = torch.where(uncertainty_est < 1e-3, uncertainty_est * 0 + 1e3, uncertainty_est)
                            uncertainty_est_real = torch.where(uncertainty_est_real < 2e-4, err_gt * 0 + 1e3, uncertainty_est_real)
                            
                            uncertainty_image_list = [normalize_result(1/uncertainty_est[0, :, :, :].data),
                                                    normalize_result(err_gt[0, :, :, :].data),
                                                    normalize_result(1/uncertainty_est_real[0, :, :, :].data)]
                            train_logger.image_summary(uncertainty_image_tag_list, uncertainty_image_list, global_step)

                        enhanced_image_tag_list = ['enhanced_gt', 'enhanced_second_est', 'enhanced_original']
                        enhanced_image_list = [enhanced_gt[0, :, :, :].data, second_predicted[0,:,:,:].data, uw_inv_normalize(image[0, :, :, :]).data]
